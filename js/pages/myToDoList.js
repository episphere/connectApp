...
export const myToDoList = async (data, fromUserProfile) => {
  console.log("myToDoList", data )
  console.log("fromUserProfile", fromUserProfile)
    const mainContent = document.getElementById('root');
    if(!data['507120821']){
        let formData = {
            '507120821':845979108 // Notes: Is this a value that does not exist anymore?
        }
        storeResponse(formData);
    }
    if(data['827220437'] && data['142654897']){
      // Healthcare provider (827220437), How did you hear about this study? (Select all that apply) (142654897)
        localStorage.eligibilityQuestionnaire = JSON.stringify({'827220437': data['827220437']})
        if(data['919254129'] === 353358909){ // Autogenerated flag - when consent form submitted (919254129)
            ...
            if(data['699625233'] && data['699625233'] === 353358909 && data['512820379'] && data['512820379'] === 854703046 && data['821247024'] && (data['821247024'] == 875007964)/*data['948195369'] && data['948195369'] !== 353358909*//*data['512820379'] && data['512820379'] !== 486306141 && data['821247024'] && data['821247024'] !== 197316935*/){
              // Notes: Autogenerated flag when User Profile submitted (699625233); Autogenerated flag - Recruitment type (512820379) === Passive (854703046); Indicator of match verification after creation of user profile - updated by sites (821247024); Not Yet Verified - (875007964)
                blockParticipant(); // Notes: Generic message display when NOT YET VERIFIED
                hideAnimation();
                return;
            }
            ...
            /*${(myData.data.hasOwnProperty('831041022') && myData.data['831041022'] == 353358909 && !myData.data['153713899']) ?`
                            ...
                            `:''}
                            */
            if(data['699625233'] && data['699625233'] === 353358909){ // Notes: Autogenerated flag when User Profile submitted (699625233)
                let template = `
                    <div class="row">
                        <div class="col-lg-2">
                        </div>
                ...
                `;
                let finalMessage = "";
                if (data.hasOwnProperty('831041022') && data['831041022'] == 353358909){ // Notes: Destroy Data flag is Yes ; Destroy data (831041022) 
                    ...
                    else if((data['747006172'] && data['747006172'] !== 104430631) && (!data['153713899'] || data['153713899'] == 104430631)){ // Withdraw consent not found/no; HIPAA revocation signed not found/no; Withdraw consent - 747006172
                        finalMessage += "You have a new <a href='#forms'>form</a> to sign.<br>You have been withdrawn from Connect per your request.<br>"
                    }
                    else{
                ...
                else if ((data['747006172'] && data['747006172'] !== 104430631)){ // Withdraw consent not found/no; HIPAA revocation signed not found/no; Withdraw consent - 747006172
                    
                    if (!data['153713899'] || data['153713899'] == 104430631){ // HIPAA revocation signed - 153713899
                        finalMessage += "You have a new <a href='#forms'>form</a> to sign.<br>You have been withdrawn from Connect per your request.<br>"
                    }
                    else{
                        finalMessage += "You have been withdrawn from Connect per your request.<br>"
                ...
                if(finalMessage.trim() !== ""){ // final message (changed) was from above conditionals
                    template += `
                ...
                else if (((data.hasOwnProperty('773707518') && data['773707518'] == 353358909)) && (!data['153713899'] || data['153713899'] == 104430631)){ // Revoke HIPAA - 773707518 Yes and HIPAA revocation signed no;     Revoke HIPAA - 773707518; HIPAA revocation signed - 153713899
                    topMessage += "You have a new <a href='#forms'>form</a> to sign.<br>"
                }

                if(!data['821247024'] || data['821247024'] == 875007964){  // Indicator of match verification after creation of user profile - updated by sites - 821247024; Not found OR value not yet verified
                if(data['unverifiedSeen'] && data['unverifiedSeen'] === true){ // Note to self: Where and when does verifiedSeen get added?*
                ...
                else if(data['821247024'] && data['821247024'] == 197316935) { // Indicator of match verification after creation of user profile - updated by sites - 821247024; value Verified
                    if(data['verifiedSeen'] && data['verifiedSeen'] === true){ //  First Survey moves to Completed
                        /*
                        topMessage += `
                            ${checkIfComplete(data) ? 'Thank you for completing your first Connect survey! We will be in touch with next steps.':'Please complete your first Connect survey.<br>Thank you for being a part of Connect.'}
                        `*/
                        
                        // if first survey data is complete and first survey complete does not exist
                        ...
                        topMessage += `
                            ...
                            <br>
                        `
                        let formData = {}; // NOTE: REFERENCE THIS CODE 
                        formData['verifiedSeen'] = true;
                        storeResponse(formData);
                    }
                }
                else if(data['821247024'] && data['821247024'] == 219863910) { // Verification status -> Cannot be verified 219863910
                    template += `
                    <div class="alert alert-warning" id="verificationMessage" style="margin-top:10px;">
                ...
                else if(data['821247024'] && data['821247024'] == 922622075) { // Verification status -> Duplicate 922622075
                    template += `
                    <div class="alert alert-warning" id="verificationMessage" style="margin-top:10px;">
                ...
                else if(data['821247024'] && data['821247024'] == 160161595) { // Verification status -> Outreach timed out
                    let siteList = sites();
...
const renderMainBody = (data, tab) => {
    ...
    modules = setModuleAttributes(data, modules); // check this for menustrual cycle set in paricipant cycle
    console.log("modules", modules)
    // d_ biospecimen, nested value
...
const checkForNewSurveys = async (data) => {
  console.log("checkForNewSurveys data", data)
    let template = ``;

    let modules = questionnaireModules();
    console.log("checkForNewSurveys modules",modules)
    modules = setModuleAttributes(data, modules); // Check to see where this will add Menustrual Triggers
    ...
    Object.keys(modules).forEach(mod => {
      console.log("modules[mod] loop", modules[mod])
        if(modules[mod].enabled && !modules[mod].completed) availableSurveys++; // Modules enabled and modules not completed, increment available surveys
    });
...
const setModuleAttributes = (data, modules) => {
    ...
    // if(data['331584571'] && data['331584571']['266600170'] && data['331584571']['266600170']['840048338']) { // Select visit from drop down menu - 331584571 ; Baseline - 266600170; Autogenerated date/time stamp for check-in complete. - 840048338
    ...
    // if(data['D_299215535'] && data['D_299215535']['D_112151599'] && data['D_299215535']['D_112151599'] == 353358909 && data['265193023'] == 231311385) { // Blood/urine/mouthwash combined research survey --> SUBMITTED
    //     modules['Menstrual Cycle'].enabled = true;
    // }
    if(true) {
        modules['Menstrual Cycle'].enabled = true; // NOTES: ENABLE CODE ABOVE WHEN DONE
    }
